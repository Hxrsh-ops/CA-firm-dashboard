...
import { MoreVertical, ArrowRight, CheckCircle, Eye } from 'lucide-react';
  ...
  return (
    <div className="bg-white border border-[#EAE6DF] rounded-xl shadow-card overflow-hidden select-none">
      ...
      {items.length > 0 ? (
                  ...
                  <td className="py-3 px-3 text-right pr-4 relative">
                    ...
                    {activeMenuId === item.id && (
                      <div
                        ...
                        className="absolute right-4 top-10 w-44 bg-white rounded-xl shadow-modal border border-[#EAE6DF] py-1.5 z-30 text-xs text-left animate-in fade-in zoom-in-95 duration-100"
                      >
                        <button
                          onClick={() => {
                          ...
                          className="w-full flex items-center gap-2 px-3 py-1.5 text-[#3D2D22] hover:bg-[#FAF8F5] transition-colors font-medium"
                        >
        ...
        <div className="p-8 text-center text-xs text-[#8C827A] space-y-1">
          <CheckCircle className="w-6 h-6 text-[#16A34A] mx-auto mb-2" />
          <p className="font-semibold text-[#2B231F]">No priority work items pending</p>
          <p>All high-priority filings and statutory documents are on schedule.</p>
        </div>
      )}
    </div>
  );
